      ...
      <section className="rounded-[26px] border border-border bg-bg/15 p-5">
        ...
        {!ttsConfigured && !configureTts ? (
          ...
          <div className="mt-5 space-y-4">
            {ttsProviders.length > 0 && <div className="flex flex-wrap items-center gap-3 rounded-2xl border border-border px-4 py-3"><label className="text-[11px] text-text-muted">{t("speech.tab.activeProvider")} <select value={ttsActive} onChange={(event) => patch({ "tts.active": event.target.value })} className="ml-2 rounded-full border border-border bg-bg px-3 py-1.5 text-xs text-text"><option value="">{t("common.none")}</option>{ttsProviders.map((provider) => <option key={provider.id} value={provider.id}>{provider.label}</option>)}</select></label><label className="ml-auto flex items-center gap-2 text-xs"><input type="checkbox" checked={autoplay} onChange={toggleAutoplay} /> {t("speech.tab.autoplayReplies")}</label><label className="w-full text-[11px] text-text-muted">{t("speech.tab.globalFallbackVoice")}<input value={String(tts.default_voice ?? "")} onChange={(event) => patch({ "tts.default_voice": event.target.value })} placeholder={t("speech.tab.fallbackVoicePlaceholder")} className="mt-1 h-9 w-full rounded-xl border border-border bg-bg/60 px-3 font-mono text-xs text-text" /></label></div>}
