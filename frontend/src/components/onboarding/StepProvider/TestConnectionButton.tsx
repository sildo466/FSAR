...
export function TestConnectionButton({ apiKeyRequired }: Props) {
  ...
  const onClick = () => {
    ...
    const timeout = window.setTimeout(() => {
      unsubscribe?.()
      setProviderField('test_result', { ok: false, error: t("onboarding.provider.testTimeout"), latency_ms: null })
      setTesting(false)
    }, 6000)
  ...
  return (
    <div className="flex items-center gap-3">
      ...
      >
        {testing ? t("onboarding.provider.testing") : t("settings.embedding.testConnection")}
      </button>
