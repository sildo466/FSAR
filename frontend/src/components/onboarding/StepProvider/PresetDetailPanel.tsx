...
export function PresetDetailPanel() {
  ...
  if (!presetId) return <div className="text-text-muted">{t("onboarding.provider.selectPresetToConfigure")}</div>

  const preset = findPreset(presets, presetId)
  if (!preset) return <div data-testid="preset-detail-loading">{t("onboarding.provider.loadingPreset")}</div>
