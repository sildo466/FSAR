...
const STEP_LABEL_KEYS = [
  'onboarding.wizard.stepProvider',
    ...
    <div className="fixed inset-0 z-50 flex flex-col overflow-hidden bg-bg/95 before:absolute before:inset-0 before:bg-[radial-gradient(circle_at_15%_10%,var(--glow-soft),transparent_34%),radial-gradient(circle_at_85%_90%,var(--glow-faint),transparent_36%)] before:content-['']">
      <div className="relative z-10 flex items-center gap-6 px-8 py-6">
        <div className="flex flex-col gap-1">
          <h1 className="font-display text-xl italic">{t("onboarding.wizard.title")}</h1>
          <p className="text-caption text-text-muted">{t("onboarding.wizard.subtitle")}</p>
        </div>
          ...
          {STEP_LABEL_KEYS.map((labelKey, i) => {
            ...
            return (
              <div key={i} className="flex items-center gap-2">
                ...
                />
                <span className="text-caption text-text-muted">{t(labelKey)}</span>
              </div>
            )
          })}
        </div>
        <Pill variant="ghost" size="sm" onClick={() => setConfirmingSkip(true)}>{t("onboarding.skip")}</Pill>
      </div>
        ...
        {confirmingSkip && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="absolute inset-0 z-20 flex items-center justify-center bg-bg/45 p-6 backdrop-blur-sm">
            <motion.div initial={{ scale: .96, y: 10 }} animate={{ scale: 1, y: 0 }} exit={{ scale: .96, y: 10 }} className="glass-strong w-full max-w-sm rounded-[28px] p-6 shadow-[0_24px_80px_var(--glow-faint)]">
              <p className="font-display text-2xl italic">{t("onboarding.wizard.skipTitle")}</p>
              <p className="mt-3 text-sm leading-6 text-text-muted">{t("onboarding.wizard.skipDesc")}</p>
              <div className="mt-6 flex justify-end gap-2"><Pill variant="ghost" onClick={() => setConfirmingSkip(false)}>{t("onboarding.wizard.keepSettingUp")}</Pill><Pill variant="primary" onClick={onSkipSetup}>{t("onboarding.skip")}</Pill></div>
            </motion.div>
          </motion.div>
        )}
      </AnimatePresence>
