...
export function StepEmbedding() {
  ...
  return (
    <div className="mx-auto flex max-w-3xl flex-col gap-5">
      <h2 className="font-display text-3xl italic">Give memory a way to find itself</h2>
      <p className="text-caption text-text-muted max-w-2xl">
        ...
        and experiences when answering you. Pick a provider, or skip to add later.
      </p>
        ...
        <motion.div key={data.provider} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -6 }}><Capsule className="flex flex-col gap-4" data-testid="embedder-detail-panel">
          {def.defaults.needs_key && (
            <div className="flex flex-col gap-1">
              <label className="text-caption text-text-muted" htmlFor="embedder-api-key">API key</label>
              <Input
                id="embedder-api-key"
                type="password"
                value={data.api_key}
          ...
          <div className="flex flex-col gap-1">
            <label className="text-caption text-text-muted" htmlFor="embedder-base-url">Base URL</label>
            <Input
          ...
          <div className="flex flex-col gap-1">
            <label className="text-caption text-text-muted" htmlFor="embedder-model">Model</label>
            <Input
              id="embedder-model"
              type="text"
              value={data.model}
            ...
            {data.probe_result && (
              <span className="text-caption" data-testid="embedder-probe-result">
                {data.probe_result.ok
                  ? `OK (dim ${data.probe_result.dim ?? '?'})`
                  : `failed: ${data.probe_result.error ?? 'unknown'}`}
              </span>
            )}
          </div>
        </Capsule></motion.div>
