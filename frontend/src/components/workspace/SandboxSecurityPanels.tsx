  ...
  return (
    <>
      ...
      <section className="flex flex-col gap-3 border-t border-border pt-6">
        ...
        {custom.length > 0 && <div className="flex flex-wrap gap-2">{custom.map((item) => <span key={item} className="flex items-center gap-2 rounded-full bg-[var(--chip-bg)] px-3 py-1.5 font-mono text-[10px]">{item}<button onClick={() => send({ type: "sensitive.remove_custom", pattern: item })}><X size={11} /></button></span>)}</div>}
      </section>
      <section className="flex flex-col gap-3 border-t border-border pt-6">
