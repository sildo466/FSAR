...
export function ProviderModal({ open, initial, existingIds, onClose, onSaved }: Props) {
  ...
  const [maxOutputTokens, setMaxOutputTokens] = useState(String(initial?.max_output_tokens ?? 12800));
  ...
  useEffect(() => {
    if (open) {
      ...
      setMaxOutputTokens(String(initial?.max_output_tokens ?? 12800));
      setModels([]);
  ...
  function save() {
    ...
    const next: Provider = {
      ...
      max_output_tokens: Number(maxOutputTokens) || 12800,
      pricing: {
