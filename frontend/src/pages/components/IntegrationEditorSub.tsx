  ...
  return (
    ...
    >
      <div className="flex items-center gap-3">
        ...
        />
        <div className="flex rounded-full bg-[var(--chip-bg)] p-1 text-[10px] font-mono uppercase tracking-[0.12em]">
      ...
      {sub.kind === "integration" ? (
        <div className="mt-4 flex items-center gap-3 rounded-2xl bg-[var(--chip-bg)] px-3 py-2">
        ...
        <div className="mt-4 grid gap-3 md:grid-cols-2">
          <label className="md:col-span-2 text-[10px] font-mono uppercase tracking-[0.14em] text-text-muted">
            ...
            <input aria-label={t("integration.subBaseUrl")} value={sub.model?.base_url ?? ""} onChange={(event) => updateModel({ base_url: event.target.value })} placeholder="https://api.example.com/v1" className="mt-1 w-full rounded-xl bg-[var(--input-bg)] px-3 py-2 font-mono text-[12px] normal-case tracking-normal text-text outline-none" />
          </label>
          <label className="text-[10px] font-mono uppercase tracking-[0.14em] text-text-muted">
            {t("integration.providerName")}
            <input aria-label={t("integration.subProviderName")} value={sub.model?.provider ?? ""} onChange={(event) => updateModel({ provider: event.target.value })} className="mt-1 w-full rounded-xl bg-[var(--input-bg)] px-3 py-2 text-[12px] normal-case tracking-normal text-text outline-none" />
          </label>
          <label className="text-[10px] font-mono uppercase tracking-[0.14em] text-text-muted">
            {t("integration.modelId")}
            <input aria-label={t("integration.subModelId")} value={sub.model?.model ?? ""} onChange={(event) => updateModel({ model: event.target.value })} className="mt-1 w-full rounded-xl bg-[var(--input-bg)] px-3 py-2 font-mono text-[12px] normal-case tracking-normal text-text outline-none" />
          </label>
          <label className="md:col-span-2 text-[10px] font-mono uppercase tracking-[0.14em] text-text-muted">
            {t("integration.apiKey")}
            <input aria-label={t("integration.subApiKey")} type="password" autoComplete="off" value={sub.model?.api_key ?? ""} onChange={(event) => updateModel({ api_key: event.target.value })} placeholder={t("integration.apiKeyPlaceholder")} className="mt-1 w-full rounded-xl bg-[var(--input-bg)] px-3 py-2 font-mono text-[12px] normal-case tracking-normal text-text outline-none" />
          </label>
          <label className="text-[10px] font-mono uppercase tracking-[0.14em] text-text-muted">
            {t("integration.protocol")}
            <select aria-label={t("integration.subProtocol")} value={sub.model?.protocol ?? ""} onChange={(event) => updateModel({ protocol: event.target.value })} className="mt-1 w-full rounded-xl bg-[var(--input-bg)] px-3 py-2 text-[12px] normal-case tracking-normal text-text outline-none">
          ...
          <label className="md:col-span-2 text-[10px] font-mono uppercase tracking-[0.14em] text-text-muted">
            ...
            <input value={sub.model?.specialty ?? ""} onChange={(event) => updateModel({ specialty: event.target.value })} placeholder={t("integration.specialtyPlaceholder")} className="mt-1 w-full rounded-xl bg-[var(--input-bg)] px-3 py-2 text-[12px] normal-case tracking-normal text-text outline-none" />
          </label>
          <label className="md:col-span-2 text-[10px] font-mono uppercase tracking-[0.14em] text-text-muted">
            {t("integration.persona")}
            <textarea value={sub.model?.persona_prompt ?? ""} onChange={(event) => updateModel({ persona_prompt: event.target.value })} rows={3} className="mt-1 w-full resize-y rounded-xl bg-[var(--input-bg)] px-3 py-2 text-[12px] normal-case leading-relaxed tracking-normal text-text outline-none" />
          </label>
        </div>
      )}
    </div>
