    ...
    <div className="grid min-h-full gap-3 lg:grid-cols-[300px_minmax(0,1fr)]">
      <aside className="rounded-[30px] border border-[var(--border)] bg-[var(--glass)] p-3 lg:sticky lg:top-3 lg:h-[calc(100vh-1.5rem)]">
        ...
        <div className="mt-2 space-y-2">
          {items.map((item) => (
            <button key={item.id} type="button" onClick={() => setSelectedId(item.id)} className={`group w-full rounded-[22px] p-4 text-left transition ${selectedId === item.id ? "bg-[var(--button-bg)] text-[var(--button-text)] button-tex shadow-[0_12px_32px_var(--glow-soft)]" : "hover:bg-[var(--chip-bg)]"}`}>
