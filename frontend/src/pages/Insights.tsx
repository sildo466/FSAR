...
export function Insights() {
  ...
  return (
    <div className="max-w-[960px] mx-auto px-8 py-10 flex flex-col gap-10">
      <header>
        <h1 className="font-display text-2xl font-semibold tracking-[-0.01em]">{t("insights.title")}</h1>
        <p className="text-text-muted">{t("insights.subtitle")}</p>
      </header>

      <section className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <KpiCard label={t("insights.decisions")} value={kpis ? kpis.total_decisions.toLocaleString() : "—"} />
        <KpiCard label={t("insights.successRate")} value={kpis ? `${kpis.success_rate_pct}%` : "—"} />
        <KpiCard label={t("insights.totalTokens")} value={kpis ? kpis.total_tokens.toLocaleString() : "—"} />
        <KpiCard
          label={t("insights.cachedTokens")}
          value={kpis ? kpis.total_cached_tokens.toLocaleString() : "—"}
        />
      </section>

      <section>
        <SectionTitle>{t("insights.toolUsage")}</SectionTitle>
        {stats.length === 0 ? (
          <p className="text-text-muted text-sm">{t("insights.noToolCalls")}</p>
        ) : (
          <div className="border border-border rounded overflow-hidden">
            <table className="w-full text-[13px]">
              <thead className="bg-bg text-text-muted font-mono text-[11px] uppercase tracking-[0.08em]">
                <tr>
                  <th className="text-left px-3 py-2">{t("insights.colTool")}</th>
                  <th className="text-right px-3 py-2">{t("insights.colUses")}</th>
                  <th className="text-right px-3 py-2">{t("insights.colSuccess")}</th>
                  <th className="text-right px-3 py-2">{t("insights.colAvgLatency")}</th>
                </tr>
              </thead>
              <tbody>
                {stats.map((s) => (
      ...
      <section>
        <SectionTitle>{t("insights.reflections")}</SectionTitle>
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          <KpiCard label={t("insights.activeStrategies")} value={String(countStrategies(strategies))} />
          <KpiCard label={t("insights.totalPrompts")} value={kpis ? kpis.total_prompt_tokens.toLocaleString() : "—"} />
          <KpiCard label={t("insights.totalCompletions")} value={kpis ? kpis.total_completion_tokens.toLocaleString() : "—"} />
          <KpiCard label={t("insights.cachedShare")} value={kpis ? `${cachedShare(kpis)}%` : "—"} />
        </div>
        <div className="border border-border rounded p-4 mt-4">
          <pre className="font-mono text-[12.5px] whitespace-pre-wrap leading-[1.55]">
            {strategies || t("insights.noActiveStrategies")}
          </pre>
        </div>
      </section>

      <section>
        <SectionTitle>{t("insights.recentDecisions")}</SectionTitle>
        {recent.length === 0 ? (
          <p className="text-text-muted text-sm">{t("insights.noDecisions")}</p>
        ) : (
            ...
            {recent.map((d) => (
              <li key={d.id} className="px-4 py-3 flex items-center gap-4">
                <span
                  ...
                  aria-label={d.success ? t("insights.successAria") : t("insights.failureAria")}
                />
