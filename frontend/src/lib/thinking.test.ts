...
import { splitThinkBlocks, hasThinkBlocks, stripThinkBlocks } from "./thinking";
...
describe("stripThinkBlocks", () => {
    it("returns input unchanged when no think blocks present", () => {
        expect(stripThinkBlocks("hello world")).toBe("hello world");
    });
    it("returns empty string for empty input", () => {
        expect(stripThinkBlocks("")).toBe("");
    });
    it("removes a single think block and keeps surrounding prose", () => {
        expect(stripThinkBlocks("before <think>step 1</think> after")).toBe(
            "before  after"
        );
    });
    it("removes multiple think blocks", () => {
        expect(stripThinkBlocks("A<think>one</think>B<think>two</think>C")).toBe(
            "ABC"
        );
    });
    it("removes multiple think blocks with spaces", () => {
        expect(stripThinkBlocks("A <think>one</think> B <think>two</think> C")).toBe(
            "A  B  C"
        );
    });
    it("removes think-only content", () => {
        expect(stripThinkBlocks("<think>only thought</think>")).toBe("");
    });
    it("leaves unclosed (mid-stream) think tag untouched", () => {
        const mid = "halfway <think> no closer";
        expect(stripThinkBlocks(mid)).toBe(mid);
    });
});
