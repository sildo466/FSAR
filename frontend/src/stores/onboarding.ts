...
export type WizardStep =
  | "provider" | "embedding" | "character_card" | "user_card"
...
const STEPS: Array<0 | 1 | 2 | 3> = [0, 1, 2, 3];
const STEP_NAME: Record<0 | 1 | 2 | 3, WizardStep> = {
  0: "provider",
  1: "embedding",
  2: "character_card",
  3: "user_card",
...
export const useWizardState = create<WizardState>((set, get) => ({
  ...
  next: async () => {
    ...
    set({ errors: errs });
    const nextIdx = STEPS[Math.min(STEPS.indexOf(s.current_step_index) + 1, 3)] as 0 | 1 | 2 | 3;
  ...
  back: () => {
    ...
    const prevIdx = STEPS[Math.max(STEPS.indexOf(s.current_step_index) - 1, 0)] as 0 | 1 | 2 | 3;
