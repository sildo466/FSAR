...
Mirrors openclaw's `src/agents/pi-embedded-runner/google-prompt-cache.ts`:
...
def compute_match_key(
    *,
    ...
    """Stable JSON match key — schema mirrors openclaw's google-prompt-cache.ts."""
    payload = {
        "provider": provider,
        "modelId": model_id,
        "modelApi": model_api,
...
class GeminiPromptCache:
    ...
    def ensure_cached_content(
        ...
        *,
        ...
        All signature/state-digest/db side effects follow openclaw's
