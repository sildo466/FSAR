...
- `src/agents/anthropic-payload-policy.ts` — `cache_control: {type:"ephemeral"}`
  rewriting on the last system block + the trailing user turn.
- `src/agents/pi-embedded-runner/cache-ttl.ts` — per-session timestamp log for
  cache-break observability (Anthropic manages cache invalidation server-side,
  so there is no POST/PATCH client lifecycle to drive — the SDK just sends
  the markers and the server decides).
...
provider is known to support it, matching openclaw's `isLongTtlEligibleEndpoint`
heuristic.
"""
...
def is_long_ttl_eligible(base_url: str, provider_override: Optional[str] = None) -> bool:
    """Match openclaw's `isLongTtlEligibleEndpoint` semantics.
...
class AnthropicCacheLog:
    """Append-only timestamp log mirroring openclaw's cache-ttl.ts.
