...
Per-session cache state pattern (expiring map + TTL timestamp log), adapted
to fsar's Python + SQLite stack.
"""
...
class _ExpiringLRU:
    ...
    Lazy prune on read, configurable max size, per-entry `stored_at` epoch
    seconds.
    """
