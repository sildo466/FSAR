...
|---|---|
| `/help` | Show this help |
| `/memory [stats\\|sessions [N]\\|session <id>\\|delete <id>\\|search <kw>]` | Memory database |
...
| `/import <path-to-skill.md>` | Import external skill markdown |
...
def _memory(engine, args, body) -> str:
    ...
    return "Usage: `/memory [stats|sessions [N]|session <id>|delete <id>|search <kw>]`"
...
def _reflect(engine, args, body) -> str:
    ...
    client, model = engine.client_and_model()
...
def _import(engine, args, body) -> str:
    ...
    if not path:
        return "Usage: `/import <path-to-skill.md>`"
