...
async def dispatch(ws: WebSocket, msg: dict[str, Any], config: FsarConfig) -> bool:
    ...
    if t == "onboarding.complete":
        try:
            ...
            await ws.send_json({"type": "onboarding.error", "code": "bad_request", "message": str(e)})
        return True
    if t == "onboarding.skip_step":
        try:
            result = await onboarding_skip_step(config, str(msg.get("step") or ""))
            await ws.send_json(result)
        except ValueError as e:
            await ws.send_json({"type": "onboarding.error", "code": "bad_request", "message": str(e)})
        return True
...
async def onboarding_get_state(fsar_config: FsarConfig) -> dict:
    ...
    skipped_steps = fsar_config.get("onboarding.skipped_steps") or []
    return {
        "type": "onboarding.state",
        "required": not completed,
        "completed": completed,
        "completed_steps": completed_steps,
        "current_step": _compute_current_step([*completed_steps, *skipped_steps]) if not completed else None,
    }
...
async def onboarding_complete(fsar_config: FsarConfig) -> dict:
    ...
    """
    steps = list(fsar_config.get("onboarding.completed_steps") or [])
    required = [s for s in ALL_STEPS if s not in {"embedding", *SKIPPABLE_STEPS}]
...
async def onboarding_skip_step(fsar_config: FsarConfig, step: str) -> dict:
    if step not in SKIPPABLE_STEPS:
        raise ValueError(f"step is not skippable: {step!r}")
    skipped = list(fsar_config.get("onboarding.skipped_steps") or [])
    if step not in skipped:
        skipped.append(step)
    fsar_config.patch("onboarding.skipped_steps", skipped)
    fsar_config.patch("onboarding.last_step", step)
    if not fsar_config.get("onboarding.started_at"):
        fsar_config.patch("onboarding.started_at", _now_iso())
    fsar_config.save()
    return {"type": "onboarding.step_skipped", "step": step}
...
async def onboarding_reset(fsar_config: FsarConfig) -> dict:
    ...
    fsar_config.patch("onboarding.skipped_steps", [])
