...
def _get_default_character_id() -> int | None:
    """Look up the target character when LLM did not supply character_id:
    default character first, then the first available character."""
    from src.memory.cards import CardRepo
    repo = CardRepo(_resolve_repo_path())
    ch = repo.get_default_character()
    if ch is None:
        candidates = repo.list_characters()
        ch = candidates[0] if candidates else None
