...
`/import <path>` and the file becomes an active experience row. Mirrors the
SKILL.md shape closely enough that community skills can be reused with light
edits — but DB is canonical, the .md file is just the input.
...
def parse_skill_folder(path: str | Path) -> tuple[ParsedSkill, list[str]]:
    from src.tools.builtin.skill_folder import parse_skill_folder_contents

    contents = parse_skill_folder_contents(path)
    parsed = contents.parsed

    templates = {item.name: item for item in parsed.templates}
    templates.update({item.name: item for item in contents.templates})
    scripts = {item.name: item for item in parsed.scripts}
    scripts.update({item.name: item for item in contents.scripts})
    references = {item.title: item for item in parsed.references}
    references.update({item.title: item for item in contents.references})
...
def import_markdown_file(path: Path) -> tuple[str, str, dict] | None:
    ...
    from src.skills.gate import gate_skill
    from src.utils.config import get_config
    verification = gate_skill(path.parent.name, get_config(), skills_root=path.parent.parent)
    if not verification.valid:
        raise PermissionError(
            f"[BLOCKED] skill '{path.parent.name}' not reviewed ({verification.reason})"
        )
