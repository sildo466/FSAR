...
from src.skills.gate import gate_mcp
...
class MCPManager:
    def __init__(
        ...
        self._config = config or FsarConfig()
        self._servers_root = servers_root or get_fsar_home() / "mcp_servers"
        self._marker = marker or SafeMarker()
    ...
    async def start(self) -> None:
        ...
        self._server_configs = {cfg["name"]: cfg for cfg in configs}
        if not configs:
    ...
    def verify_server(self, server_name: str) -> MarkerVerification:
        cfg = self._server_configs.get(server_name)
        if cfg is None:
            return MarkerVerification(False, "server_config_missing")
        try:
            return gate_mcp(
                cfg,
                self._config,
                servers_root=self._servers_root,
                marker=self._marker,
            )
        except ValueError:
            return MarkerVerification(False, "invalid_server_name")
    ...
    async def _start_one(self, cfg: dict[str, Any]) -> None:
        ...
        try:
            ...
            log.error(f"MCP[{name}] start blocked: {error}")
            return
        if not verification.valid:
            self._started[name] = False
            log.error(
                f"MCP[{name}] start blocked: server not reviewed ({verification.reason})"
            )
            return
        if cfg["transport"] == "stdio" and self._config.get(
            "security.mcp.cwd_pinning.enabled", True
        ):
            pinned_cwd = self._servers_root / name
            require_dir = bool(
                self._config.get("security.mcp.cwd_pinning.require_dir", True)
            )
            if require_dir and not pinned_cwd.is_dir():
                self._started[name] = False
                log.error(f"MCP[{name}] start blocked: pinned cwd does not exist")
                return
            pinned_cwd.mkdir(parents=True, exist_ok=True)
            cfg = {**cfg, "cwd": str(pinned_cwd)}
