...
def test_chat_send_agent_returns_llm_text(monkeypatch):
    monkeypatch.setattr(ws_mod._engine, "client_and_model", lambda: (object(), "model-x"))
    monkeypatch.setattr(ce, "cached_chat_completion", lambda *a, **k: _resp(content="hi there"))
    monkeypatch.setattr(ws_mod._engine, "_save_user", lambda c: None)
    monkeypatch.setattr(ws_mod._engine, "_save_assistant", lambda m, c: None)
    monkeypatch.setattr(ws_mod._engine, "_reflect", lambda t, u: None)
    client = TestClient(ws_mod.app)
    with client.websocket_connect("/ws") as ws:
        ws.receive_json()
        ws.send_json({"type": "chat.send", "content": "hello", "mode": "agent"})
