...
def test_input_assistant_with_text_and_tool_calls_emits_both_typed():
    """Assistant turn with both content AND tool_calls must emit two typed
    items — function_call and a typed `message` — never a bare `{content: ...}`
    dict that the Responses API rejects with `invalid role`."""
    msgs = [
        {"role": "user", "content": "go"},
        {"role": "assistant", "content": "calling foo", "tool_calls": [
            {"id": "tc_1", "function": {"name": "foo", "arguments": "{}"}},
        ]},
    ]
    out = messages_to_responses_input(msgs)
    asst_msgs = [item for item in out
                 if item.get("type") == "message" and item.get("role") == "assistant"]
    assert len(asst_msgs) == 1
    assert asst_msgs[0]["content"] == [{"type": "output_text", "text": "calling foo"}]


def test_input_empty_assistant_still_emits_typed_message():
    """Empty assistant turn (e.g. one that only had tool_calls and got
    trimmed) must still surface as a typed message — never collapse into
    a bare dict."""
    ...
    asst_msgs = [item for item in out
                 if item.get("type") == "message" and item.get("role") == "assistant"]
    assert len(asst_msgs) == 1
    assert asst_msgs[0]["content"] == [{"type": "output_text", "text": ""}]


def test_input_multi_turn_no_untyped_items():
    """Regression: 2-turn conversation (greeting + follow-up) used to emit a
    bare `{"content": [...]}` assistant item when the prior turn's reply was
    replayed, causing the Responses API to 400 with `invalid role`."""
    msgs = [
    ...
    out = messages_to_responses_input(msgs)
    for item in out:
        # Every item must declare either a `type` (function_call[_output]) or
        # both `type: "message"` + `role`. A bare `{content: ...}` is the bug.
        t = item.get("type")
        if t in ("function_call", "function_call_output"):
            continue
        assert t == "message", f"untyped item in input: {item!r}"
        assert item.get("role") in ("user", "assistant", "system"), item
        assert "content" in item, item


def test_input_mid_history_system_preserved():
    """Mid-history system blocks (e.g. re-injected mid-conversation) must be
    kept as typed messages instead of silently dropped."""
    msgs = [
        {"role": "user", "content": "hi"},
        {"role": "assistant", "content": "hello"},
        {"role": "system", "content": "from now on reply tersely"},
        {"role": "user", "content": "ok go"},
    ]
    out = messages_to_responses_input(msgs)
    sys_items = [i for i in out if i.get("role") == "system"]
    assert len(sys_items) == 1
    assert sys_items[0]["type"] == "message"
    assert sys_items[0]["content"] == [{"type": "input_text", "text": "from now on reply tersely"}]


def test_input_mid_history_system_empty_content_skipped():
    """Empty system block mid-history still emits an empty typed-message so
    the role remains visible (matching the empty-assistant guarantee)."""
    msgs = [
        {"role": "user", "content": "hi"},
        {"role": "system", "content": ""},
        {"role": "user", "content": "go"},
    ...
    sys_items = [i for i in out if i.get("role") == "system"]
    assert len(sys_items) == 1
    assert sys_items[0]["content"] == [{"type": "input_text", "text": ""}]


def test_input_multi_turn_with_mid_system_no_untyped_items_v2():
    """Updated regression: every item in a 4-turn chat (including a
    mid-history system re-injection) must still be typed + roleed — no
    `{'content': [...]}` naked dicts slip through."""
    msgs = [
        {"role": "user", "content": "hi"},
        {"role": "assistant", "content": "hello"},
